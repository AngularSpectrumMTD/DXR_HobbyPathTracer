...
uint getGlobalRandomNumber()
{
    return gSceneParam.additional3.y;
}
    ...
    in float3 currPos, in float3 prevPos)
{
    ...
    const bool isNearNormal = (dot(currNormal, prevNormal) > 0.75f) || ((length(currNormal) == 0) && (length(prevNormal) == 0));
